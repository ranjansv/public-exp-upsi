...
then
    ...
    do
        for DATASIZE in $DATA_PER_RANK
        do
            OUTPUT_FILE="${RESULT_DIR}/csv/readworkflowtime-${NR}ranks-${DATASIZE}mb-readsize-${READ_IO_SIZE}.csv"
            echo "io_method,$AGGREGATORS,None" > $OUTPUT_FILE
            sed -i 's/\s/,/g' $OUTPUT_FILE
            for IO_NAME in $ENGINE
	    do
		...
		then
		    ...
		    READWORKFLOW_TIME=`cat $RESULT_DIR/${NR}ranks/${DATASIZE}mb/daos-array/readworkflow-time.log`
		    echo -n ",$READWORKFLOW_TIME" >> $OUTPUT_FILE
		else
                    for ADIOS_XML in $AGGREGATORS 
                    do
                            READWORKFLOW_TIME=`cat $RESULT_DIR/${NR}ranks/${DATASIZE}mb/${IO_NAME}/${ADIOS_XML}/readworkflow-time.log`
                            echo -n ",$READWORKFLOW_TIME" >> $OUTPUT_FILE
                    done
                    echo -n "," >> $OUTPUT_FILE
		fi
    ...
    do
        for DATASIZE in $DATA_PER_RANK
        do
            OUTPUT_FILE="${RESULT_DIR}/csv/avgreadtime-${NR}ranks-${DATASIZE}mb-readsize-${READ_IO_SIZE}.csv"
            echo "io_method,$AGGREGATORS,None" > $OUTPUT_FILE
            sed -i 's/\s/,/g' $OUTPUT_FILE
            for IO_NAME in $ENGINE
	    do
		...
		then
		    ...
		    READ_TIME=`grep 'read-time' $RESULT_DIR/${NR}ranks/${DATASIZE}mb/daos-array/stdout-mpirun-readers.log|awk '{printf "%.2f", $4}'`
