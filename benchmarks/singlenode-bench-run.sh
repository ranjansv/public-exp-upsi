  	    ...
  	    echo "global array size: $GLOBAL_ARRAY_SIZE"
		   ...
		   mkdir -p $OUTPUT_DIR
		       ...
		       echo "Starting readers with read io size(bytes): $IOSIZE"
  	               START_TIME=$SECONDS
                         ibrun -o 0 -n $NR_READERS numactl --cpunodebind=0 --preferred=0 env CALI_CONFIG=runtime-report,calc.inclusive  build/daos_array-reader $POOL_UUID $CONT_UUID $GLOBAL_ARRAY_SIZE $IOSIZE $STEPS &>> $OUTPUT_DIR/stdout-mpirun-readers-iosize-$IOSIZE.log
  	               ELAPSED_TIME=$(($SECONDS - $START_TIME))
		   ...
		   then
  	       ...
  	       elif [ $ENG_TYPE == "adios+daos-posix" ]
  	       then
  		   echo "Destroying previous containers, if any "
  		   daos pool list-cont --pool=$POOL_UUID |sed -e '1,2d'|awk '{print $1}'|xargs -L 1 -I '{}' sh -c "daos cont destroy --cont={} --pool=$POOL_UUID --force"
		          ...
		          do
		              echo "Starting readers with read io size(bytes): $IOSIZE"
  	                      START_TIME=$SECONDS
                              ...
                              ibrun -o 0 -n $NR_READERS  numactl --cpunodebind=0 --preferred=0  env CALI_CONFIG=runtime-report,calc.inclusive LD_PRELOAD=$PRELOAD_LIBPATH build/reader posix $FILENAME $READ_IO_SIZE &>> $OUTPUT_DIR/stdout-mpirun-readers-iosize-$IOSIZE.log
  	                      ELAPSED_TIME=$(($SECONDS - $START_TIME))
  	                  echo "$ELAPSED_TIME" > $OUTPUT_DIR/readworkflow-iosize-$IOSIZE-time.log
			  done
		   ...
		   mkdir -p $OUTPUT_DIR
		      ...
		      then
		          if [ $NR -ne $NR_READERS ]
			  then
			      echo "ior+dfs is not currently setup for unequal number of readers and writers"
			      continue
			  fi
		          echo "Starting IOR writers"
  	                  START_TIME=$SECONDS
                          ibrun -o 0 -n $NR  numactl --cpunodebind=0 --preferred=0 ior -a DFS -b ${DATASIZE}mb -t ${DATASIZE}mb -v -W -w -k  -o /testfile --dfs.pool $POOL_UUID --dfs.cont $CONT_UUID &>> $OUTPUT_DIR/stdout-mpirun-writers.log 
  	                  ELAPSED_TIME=$(($SECONDS - $START_TIME))
  	                  echo "$ELAPSED_TIME" > $OUTPUT_DIR/writeworkflow-time.log
		          for IOSIZE in $READ_IO_SIZE
		          do
		              echo "Starting IOR readers with read io size(bytes): $IOSIZE"
  	                      START_TIME=$SECONDS
                              ibrun -o 0 -n $NR_READERS  numactl --cpunodebind=0 --preferred=0 ior -a DFS -b ${DATASIZE}mb -t $IOSIZE -v -r -i $STEPS -k  -o /testfile --dfs.pool $POOL_UUID --dfs.cont $CONT_UUID &>> $OUTPUT_DIR/stdout-mpirun-readers-iosize-$IOSIZE.log
  	                      ELAPSED_TIME=$(($SECONDS - $START_TIME))
  	                  echo "$ELAPSED_TIME" > $OUTPUT_DIR/readworkflow-iosize-$IOSIZE-time.log
			  done
		      else
