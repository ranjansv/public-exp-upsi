...
SCRIPT_NAME="bench-writer-only.sh"
	...
	for IO_NAME in $ENGINE; do
		...
		for DATASIZE in $DATA_PER_RANK; do
			...
			if [ $ENG_TYPE == "daos-array" ]; then
				...
				mkdir -p $OUTPUT_DIR
				if [ $BENCH_TYPE == "writer-only" ]; then
					START_TIME=$SECONDS
				...
				if [ $BENCH_TYPE == "writer-only" ]; then
					echo "Starting IOR writers"
					START_TIME=$SECONDS
					ibrun -o 0 -n $NR numactl --cpunodebind=0 --preferred=0 ior -a DFS -b ${DATASIZE}mb -t ${DATASIZE}mb -v -w -i $STEPS -k -o $FILENAME --dfs.pool $POOL_UUID --dfs.cont $CONT_UUID &>>$OUTPUT_DIR/stdout-mpirun-writers.log
					ELAPSED_TIME=$(($SECONDS - $START_TIME))
					echo "$ELAPSED_TIME" >$OUTPUT_DIR/writeworkflow-time.log
				else
